...
#sys.path.append(OBJECT_DETECTION_PATH) #Need to append path with the object detection stuff before imports below
...
## LOAD CONFIG PARAMS ##
os.chdir(os.path.dirname(os.path.realpath(sys.argv[0]))) #Change working directory to where the script is, because roslaunch does weird stuff
...
def load_frozenmodel():
    ...
    if not split_model:
        ...
        print("Creating detection graph")
        detection_graph = tf.Graph()
        with detection_graph.as_default():
          od_graph_def = tf.GraphDef()
          with tf.gfile.GFile(model_path, 'rb') as fid:
...
def detection(detection_graph, category_index, score, expand):
    ...
    with detection_graph.as_default():
        with tf.Session(graph=detection_graph,config=config) as sess:
            ...
            while True:
                if not doLoop: #Do this instead of while doLoop because of the continue usage
                    break
                if split_model:
                    # split model in seperate gpu and cpu session threads
                    if len(imgStack) and gpu_worker.is_sess_empty():
                        data = imgStack.pop()
...
def ImageCallback(data):
    imgStack[:] = [] #Clear the list. Clear function doesn't exist in Python 2
...
def main():
    ...
    image_sub = rospy.Subscriber("/camera/left/image_raw", Image, ImageCallback)
    signal.signal(signal.SIGINT, SignalHandler)

    #Create warmup data
    warmupImg = cv2.imread("/home/ubuntu/slam/Kitti00/image_0/0.png", 1)
    warmupImgMsg = bridge.cv2_to_imgmsg(warmupImg, 'rgb8')
    imgStack.append(warmupImgMsg)
